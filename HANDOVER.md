...
| **`_assigns_field` does not match compound assignment** | The helper is documented as signalling a step that **populates state**. `total += x` populates state and is not matched — the regex is `name\s*=(?!=)`, so the `+` blocks it. Phase 2 step 2 deliberately preserves this (exact parity), because a mechanism change that *also* changes meaning makes any resulting movement ambiguous: you could not say whether a moved verdict came from the tree or from the new meaning. **Candidate meaning change, its own prediction, after the migration.** Corpus coverage today: 0 sites. |
